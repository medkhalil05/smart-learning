...
// Deterministic pseudo-random generator (mulberry32) — same output on server and client.
// This avoids the React hydration mismatch caused by Math.random().
...
export const heatmapData = Array.from({ length: 50 }, (_, i) => ({
  page: i + 1,
  difficulty: Math.round(_rng() * 100),
  errors: Math.floor(_rng() * 30),
}));
